...
6. 个人数据查询依赖“玩家身份映射”，因此首版必须引入昵称绑定机制。
...
- `query_service.py`：处理个人战绩和最近比赛。
- `ranking_service.py`：处理群排行。
- `identity_service.py`：处理玩家绑定与别名映射。
...
需求文档中的业务目标要求用户可以查询“自己的”战绩和统计，但截图中出现的是游戏昵称，不一定等于群昵称，也不一定能直接映射到平台用户 ID。
...
- `/网球 绑定 <游戏昵称>`：把当前群成员与某个游戏昵称绑定。
- `/网球 别名`：查看自己当前绑定的游戏昵称列表。

后续录入时，系统优先用绑定关系解析截图中的玩家身份。
...
3. 插件返回识别预览，包含：
   ...
   - 身份解析结果
   - 是否存在缺失字段
4. 插件提示用户发送 `/网球 确认 <记录号>` 或 `/网球 取消 <记录号>`。

首版不依赖多轮会话等待器完成确认，避免会话超时与上下文丢失问题。确认仍通过显式命令完成。
...
用户发命令和截图
  ...
  -> IdentityService 尝试解析双方玩家
  -> DuplicateDetector 检查是否疑似重复
  -> Repository 写入 pending 记录
  -> ResultRenderer 输出预览
```
...
身份映射用于解决“截图中的游戏昵称”和“群成员身份”之间的绑定问题。

### 8.2 映射策略

身份解析优先级如下：

1. 根据 `@对手` 参数和提交人身份，优先尝试绑定双方。
2. 根据当前群的已知别名库匹配截图中的昵称。
3. 如果提交人尚未绑定昵称，但截图中仅有一个昵称能合理归属于提交人，可提示确认绑定。
4. 如果仍无法确定双方身份，记录保留为 `pending`，但不允许确认入库。

### 8.3 关键规则

- 昵称匹配应先做标准化，例如去首尾空格、统一大小写。
- 同一群内允许一个玩家拥有多个别名。
- 同一群内同一个标准化昵称只能绑定给一个玩家。
- 只有身份解析完成的比赛才允许进入正式统计。
...
| `id` | INTEGER PK | 主键 |
| `group_id` | INTEGER FK | 所属群 |
| `player_id` | INTEGER FK | 玩家 |
| `alias` | TEXT | 原始别名 |
| `normalized_alias` | TEXT | 标准化别名 |
| `created_at` | DATETIME | 创建时间 |

约束：

- 唯一索引：`(player_id, normalized_alias)`
- 唯一索引：`(group_id, normalized_alias)`

这样可以在数据库层直接保证“同一群内同一个标准化昵称只能归属于一个玩家”。
